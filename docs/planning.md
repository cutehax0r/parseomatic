...
- **Units get a two-level split**, same shape as the existing player-metadata idea: a **GUID table** (one entry per unique spawn instance — a GUID is unique even when two units share a name, e.g. two wolves both named "Timber Wolf") holding a name-table index plus metadata (type, class, owner GUID for pets), and a separate **name table** (deduplicated display strings) that many GUIDs point at. Spells follow the same idea: Blizzard's own numeric spellId is sparse (six digits, mostly unused), so it's a `HashMap` key into a **dense local index**, not used directly as an array index.
  - **Player unit names carry a realm suffix** (`"Character-Realm-Region"`, e.g. `Aanx-DarkIron-US`). Interning splits on the first `-`: the character half goes in `name_id`, the `"Realm-Region"` half in a separate `server_id` (both into the shared name table — realms repeat heavily). Non-player names are stored verbatim. Kept apart in the struct so a view can show just the name, or `"Name – Server"`, without re-splitting a string every render.
...
### Per-encounter derived stats
`src-tauri/src/stats.rs` reduces each encounter's event window (once, in parallel, lazily on first request) to per-player `EncounterStats`: damage/healing (own vs pet), damage taken, deaths, alive time, **active time** (via a pluggable `ActivityModel`), and movement (distance, moving time, per-decile bins). One scan per encounter, so a boss selection in the Overview is an O(1) lookup rather than repeated live query scans. See `docs/activity-and-movement.md`.
