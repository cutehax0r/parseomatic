...
|---|---|
| `_DAMAGE` | amount, overkill, school, resisted, blocked, absorbed, critical, glancing, crushing, isOffHand — **confirmed exactly 10 fields against the fixture**, no `baseAmount` insertion (see resolved-gotcha below). `SPELL_DAMAGE`/`SPELL_PERIODIC_DAMAGE`/`RANGE_DAMAGE` (spell-prefixed) additionally carry a **new 11th trailing field, `hitType`** (`ST` or `AOE` observed) — see new-field note below |
...
- **RESOLVED — `_DAMAGE` field count.** The wiki's `baseAmount`-insertion theory was **wrong**, at least for patch 12.0.7: confirmed exactly 10 fields for `SWING_DAMAGE`/`SWING_DAMAGE_LANDED` (no spell prefix, so no ambiguity about where prefix ends) by exact arithmetic against the fixture (`total fields − 9 base − 19 advanced = 10`). No `baseAmount` field observed anywhere. Trust the plain 10-field table above.
...
**Not observed in the fixture at all** (`grep -c COMBATANT_INFO` → 0, despite the file containing real `ENCOUNTER_START`/`ENCOUNTER_END` pairs). Don't read too much into this — it's one log from one client, and this doc's own §1 already established this client's header (`ADVANCED_LOG_ENABLED,0`) doesn't reliably reflect what's actually written, so a missing `COMBATANT_INFO` here isn't strong evidence it's gone from the format. But it does mean **this doc's `COMBATANT_INFO` field list below is still wiki-sourced only, not yet cross-checked against a real patch-12.x example** — treat the structural gaps already flagged (hero/Apex talent field 27 shape) as still fully open, and get a log with combatant info present before hardcoding this section.
...
> **Patch-12 gap (checked 2026-08-30, still open, now confirmed rather than guessed):** re-checked specifically for a hero-talent/Apex-talent-era (patch 11.0.2+ hero talents, patch 12.0.1+ Apex talents per `Patch_12.0.1`) variant of this field. The `Event:COMBAT_LOG_EVENT` page — despite being actively edited as recently as 2026-08-29 — **still only documents the pre-Dragonflight (Artifact Traits) and Shadowlands (soulbind/covenant) shapes** for field 27; there's no third documented variant, and no separate field anywhere in the documented COMBATANT_INFO layout for hero talents or Apex talents. The wiki's own "Class Talents" field (25) is a flat, variable-length list of talent/spell IDs and is generic enough that it's *plausible* hero talents and Apex talents are just additional entries in that same flat list rather than a new field — but the wiki does not say this explicitly anywhere, so treat it as **unconfirmed, not resolved**. Parseomatic should not assume a fixed field count for field 27 or a fixed talent-list length for field 25; verify against a real patch-12.x `COMBATANT_INFO` line before hardcoding structure here.
...
10. **RESOLVED — `_DAMAGE` suffix is exactly 10 fields, no `baseAmount` insertion.** The wiki's theory was wrong for this patch; confirmed by exact arithmetic against `SWING_DAMAGE`/`SWING_DAMAGE_LANDED` samples (§7).
...
21. **One gap remains genuinely open**: whether hero talents / Apex talents get a new `COMBATANT_INFO` field or fold into the existing flat `Class Talents` list — moot until a fixture with `COMBATANT_INFO` present is available (§8, item 6 above).
